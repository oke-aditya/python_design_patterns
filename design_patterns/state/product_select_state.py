...
class ProductSelectState(VendingState):
    def choose_product(self, vending_machine: 'VendingMachine', product_code: str) -> None:
        ...
        if(user_price < price):
            ...
            raise ValueError(f"Given money = {user_price} is less than price of product = {price} ")
        
        else:
            change = user_price - price
            # Simplifying the logic a bit here
            vending_machine.set_coins([change])
            vending_machine.set_vending_machine_state(DispenseProductState(vending_machine))
